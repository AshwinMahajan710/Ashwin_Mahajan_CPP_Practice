...
void Time::setHour( const int hr){
    this->hour = (hr>=24 || hr<0) ? throw(std::invalid_argument("The Entered value is not valid hour...")) : hr;
}; 

//  Validate and then set minute
void Time::setMinute( const int minute){
    this->minute = (minute>=60 || minute<0) ? throw(std::invalid_argument("The Entered value is not valid minute...")):minute;
} 

//  Validate and then set second
void Time::setSecond(const int sec){
    this->second = (sec>=60 || sec<0) ? throw(std::invalid_argument("The Entered value is not valid second...")):sec;
} 
...
void Time::tick(){
    ...
    std::cout<<"Time incremented..."<<std::endl;
    printUniversal();
}
